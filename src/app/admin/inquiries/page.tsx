          ...
          {rows.map((row) => (
            <li key={row.id}>
              <Link href={`/admin/inquiries/${row.id}`} className="flex items-center justify-between gap-4 px-4 py-4 hover:bg-ink-05">
                <div>
                  <p className="adm-body text-forest">
                    {maskName(row.name)}
                    {isLiftLead(row) ? (
                      <span className="adm-meta ml-2 rounded-full bg-grass px-2 py-0.5 text-forest">
                        LIFT
                      </span>
                    ) : null}
                  </p>
                  <p className="adm-meta mt-1 text-ink-70">
                    ...
                    {row.preferredTime && !isLiftLead(row) ? ` · ${row.preferredTime}` : ""}
                  </p>
                </div>
